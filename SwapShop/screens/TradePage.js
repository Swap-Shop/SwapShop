...
const TradePage = ({navigation}) => 
{
  ...
  const Reject = (postID) => { // this code is used to delete data from the database. 
      firestore()
        .collection('Trade')
        .doc(postID)
        .update({
    ...
    const Accept = (postID) => { // this code is used to delete data from the database. 
      firestore()
        .collection('Trade')
        .doc(postID)
        .update({
    ...
    const handleReject = (postID) => { // this is an interactive alert box that is used to confirm whether a user is sure about deleting a post
      Alert.alert(
        'Reject the request',
        'Are you sure?',
        [
    ...
    const handleAccept = (postID) => { // this is an interactive alert box that is used to confirm whether a user is sure about deleting a post
      Alert.alert(
        'Accept the request',
        'Are you sure?',
        [
    ...
    const ViewItem = (ProductID, CustomerID, OwnerID) => { // this code is used to delete data from the database. 
    ...
    return(
      <Container style={{   backgroundColor: '#F2F3F4',}}>
              <SafeAreaView style={{height: 50}}>
                ...
                <Text style={{color: 'black', fontWeight: 'bold', fontStyle: 'italic'}}> View your trade requests </Text>
              </TouchableOpacity>
              </SafeAreaView>
              <FlatList
              data={posts}
